...
load_dotenv() #procura um arquivo .env com váriáveis
DB_PATH = os.getenv('DATABASE', './data/desejos.sqlite3')

def init_db(db_name: str = DB_PATH) -> None:
    with connect(db_name) as conn:
        conn.execute("""
        CREATE TABLE IF NOT EXISTS streamings (
            id_streaming INTEGER PRIMARY KEY AUTOINCREMENT,
            titulo_streaming TEXT NOT NULL,
            tipo_streaming TEXT,
            indicado_por TEXT,
            imagem TEXT
        );
